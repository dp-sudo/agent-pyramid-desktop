...
当前 F:\Agent 项目的 renderer 处于"破而未立"状态：HEAD 中 `App.tsx` 与 `styles.css` 已删，`main.tsx` 仍引用，`ui/components/{composer,topbar,sidebar,settings,main,icons}` 全是空目录。早期 `window.agentApi.run()` 是单回合 IPC —— 用户每次都是新任务，无 thread 列表、无消息流、无工具调用、无中断恢复；该旧入口已在多 turn runtime 落地后下线。
...
- 扩展 `src/shared/agent-contracts.ts`：新增 `ThreadRecord / TurnRecord / Item / RuntimeEvent / ApprovalRequest` 类型；删除旧单回合调用的兼容壳。
...
- 扩展 `src/preload/index.ts`：暴露 `window.agentApi.{threads, turns, sse, approvals, write}`，删除 `window.agentApi.run` 旧签名。
- 改造 `src/main/application/agent-runtime.ts`：实现多 turn 编排；把 LLM 推理调用迁出主进程到 `src/main/infrastructure/llm-worker/worker.ts`（Node `worker_threads`），主进程只做编排。
...
- 扩充 `src/renderer/src/i18n/locales/{en,zh-CN}/translation.json`：删除旧单回合控制台命名空间，保留当前 Workbench 使用的 `chat / write / threads / inspector / approvals / common / composer / settings / routes / usage / empty` 命名空间。
...
- **代码文件**：
  ...
  - 修改：`shared/agent-contracts.ts`、`shared/ipc.ts`、`preload/index.ts`、`main/index.ts`、`main/application/agent-runtime.ts`、`renderer/src/main.tsx`、`renderer/src/i18n/index.ts`、两个 `translation.json`。
...
- **IPC 表面**：12+ channel；旧 `agentApi.run()` 已下线，外部调用统一使用 thread / turn / SSE API。
...
- **风险**：
  ...
  - R4（低）：删除旧 `run()` 兼容壳会破坏仍依赖旧 IPC 的外部调用方；当前项目以清理弃用代码和单一路径维护为优先。
