...
本仓库是 `agent-pyramid-desktop`：基于 Electron、Vite、React、TypeScript 的桌面 Agent Workbench。当前实现是“主进程运行时 + worker 隔离 LLM HTTP + preload 安全桥 + React 渲染端 + checkpoint / MCP / skills 扩展能力 + Vitest 覆盖关键逻辑”的桌面应用。
...
- `src/main/index.ts` 是主进程组合根，组装 `JsonlThreadStore`、`AttachmentStore`、`ModelConfigStore`、`RuntimePreferencesStore`、`CheckpointStore`、`RuntimeEventBus`、`LlmWorkerPool`、`AgentRuntime`、`SkillService`、`McpCacheStore`、`McpHost`、`InMemoryToolRegistry` 和全部 IPC handler。
...
- `src/main/application/agent-runtime.ts`：当前主运行时。负责多 turn 编排、线程历史收集、模型 profile 选择、附件上下文、skills 动态上下文、checkpoint recorder 注入、上下文预算压缩、worker 调用、流式 item 更新、工具循环、approval gate、中断、goal 更新和事件广播。
- `src/main/application/tools/`：工具注册与内置工具。`InMemoryToolRegistry` 是当前注册表；`createPlanTool`、`createWorkspaceTools()`、`createCodingTools()`、`createCommandTools()`、`createSkillTools()`、`createGoalTools()` 是当前 runtime 可用工具来源。
- `src/main/skills/`：workspace skills 服务，负责扫描约定根和额外根、追加内置 skills、暴露 catalog 诊断，并为 runtime 解析当前 turn 的 inline / subagent skill。
...
- `src/main/infrastructure/mcp/`：MCP host/client/cache/transport 实现，按 `RuntimePreferences.mcpServers` 连接 stdio 或 Streamable HTTP server，并把外部 tools 动态注册为 `mcp__<server>__<tool>`。
- `src/main/ipc/`：主进程 IPC handler，按 `threads`、`turns`、`sse`、`approvals`、`goals`、`attachments`、`usage`、`checkpoints`、`workspace`、`write`、`model-config`、`runtime-preferences`、`skills`、`mcp` 分文件注册。
- `src/main/persistence/`：Electron `userData` 下的线程 JSONL、附件、共享 config、checkpoint 与 MCP cache 持久化。
- `src/main/event-bus.ts`：运行时事件总线，按 `RuntimeEventKind` 和 `threadId` / process-level event 订阅。

### 5.2 共享契约

- `src/shared/agent-contracts.ts` 是跨 main / preload / renderer 的权威类型来源，包含模型配置、runtime preferences、thread、turn、item、runtime event、approval、checkpoint、skills、MCP、write-mode、IPC envelope 与类型守卫。
- `src/shared/ipc.ts` 是 IPC channel 名称的权威来源，`RENDERER_TO_MAIN_CHANNELS` 是 renderer 可调用 channel 清单。
- `src/shared/ipc-errors.ts` 是 IPC 错误码的权威来源。
- `src/shared/locale.ts` 是支持语言的权威来源。
- `src/shared/skills/` 是 `SKILL.md` manifest、registry、内置 skills 和解析规则的权威来源。
...
- `sse.*`：subscribe / unsubscribe / subscribeGlobal / unsubscribeGlobal / onEvent。
- `approvals.*`：respond。
- `goals.*`：update。
- `attachments.*`：create / get / delete。
- `usage.*`：daily。
- `checkpoints.*`：list / rewind。
- `mcp.*`：listServers / connect / disconnect / listTools / refreshTools / refreshSurface / listPrompts / getPrompt / listResources / readResource。
- `workspace.*`：pickDirectory。
- `write.*`：list / get / put / create / rename / delete / complete。
...
- `src/renderer/src/ui/components/settings/`：设置页导航、搜索与基础控件。
...
- 负责流式输出、JSONL 持久化、模型 profile 解析、附件注入、skills 注入、checkpoint recorder 注入、上下文预算压缩、approval gate、多轮工具调用和中断。
...
5. Runtime replay thread 历史，按当前 turn 的 plan / goal 模式拼接系统上下文；当 `RuntimePreferences.skills.enabled` 为 true 时，`SkillService` 会扫描 workspace skills、匹配当前 turn 并注入预算内的动态 skill system context；附件会通过 `AgentContentBlock[]` 注入为 text + image block。
6. Runtime 组装 `LlmRequest` 前调用 `context-compaction.prepareMessagesForRequest()`，按 `model_auto_compact_token_limit`、`model_context_window`、`max_tokens` 计算输入预算，并执行工具结果裁剪、旧消息裁剪与强制压缩。
...
10. 如模型返回工具调用，runtime 进入自动工具循环。自动工具预算由模型 profile 的 `agent_autonomy` 决定：`conservative` 为 12、`balanced` 为 32、`deep` 为 64；`AGENT_MAX_TOOL_ROUNDS` 可在 1 到 128 范围覆盖。超过预算会写入 failed `ToolItem` 和 warning `SystemItem`，emit `tool_budget_reached`，并把 turn 标记为 `needs_continuation`。
11. Runtime 为每个工具创建 `ToolItem` 并执行；checkpoint recorder 会注入写入工具上下文以记录每个 turn/file 的 pre-edit 与 post-write 快照；`list_files` / `read_file` / `search_files`、只读 developer 工具、`list_skills` / `run_skill` 是只读工具且免 approval，`create_plan` 只在 plan mode 暴露并免 approval，`update_goal` 只在 goal mode 或 active goal thread 暴露并免 approval，其它工具调用按 sandbox、approval policy、permissionRules 和 tool metadata 决定是否请求 approval。
...
- Renderer 调用 `turns.interrupt(turnId)`。
...
- 当前注册工具包括 `createPlanTool`、`createWorkspaceTools()` 返回的 `list_files` / `read_file` / `search_files`，`createCodingTools()` 返回的 `edit_file` / `write_file` / `delete_file` / `apply_patch` / `rollback_file`，`createCommandTools()` 返回的 shell、Git、package/task、command session、诊断与搜索工具，`createSkillTools()` 返回的 `list_skills` / `run_skill`，以及 `createGoalTools()` 返回的 `update_goal`。MCP 工具由 `McpHost` 动态注册为 `mcp__<server>__<tool>`。
- 只读 workspace / developer / skill 工具和 `diagnose_file` 跳过 approval；`create_plan` / `update_goal` 由模式门控并跳过 approval；coding 写入工具、foreground shell/package/Git/session 写入工具、`diagnose_workspace` 和非 read-only MCP 工具进入 approval gate。
- Write threads 默认通过 `ToolCatalogService` 隐藏并拒绝 Code-only coding/command 工具；tool access policy 可按 `code` / `write` mode allow 或 deny 指定工具名，但 approval 和 sandbox 策略仍在后续执行前生效。
- `RuntimePreferences.permissionRules` 可按 command / write / MCP 规则设置 `allow | ask | deny`，由 `ToolPolicyService` 在 hard read-only sandbox 和 `approvalPolicy: "never"` 拒绝之后评估。
- Workspace 工具和 Write IPC 文件服务共享 workspace path policy，只允许访问 active thread 的 `workspace`，使用 lexical + realpath 校验防 path escape，并跳过隐藏目录、`DeepSeek`、`dist`、`node_modules`、`out` 等目录。
- 修改工具目录、审批或权限策略时必须同步 `ToolCatalogService`、`ToolPolicyService`、`ApprovalCoordinator`、UI 展示、持久化记录和相关测试。
...
LLM 协议依据以 gateway 源码、测试和任务提供的 provider 文档为准。外部协议资料只能作为只读参考，不得 import 到运行时代码，也不得加入构建链路。
...
- `userData/config` 是模型 profiles 与 runtime preferences 的共享配置文件；`ModelConfigStore` 与 `RuntimePreferencesStore` 必须通过共享 config writer 写入，避免互相覆盖配置段。

运行时偏好由 `RuntimePreferencesStore` 写入 Electron `userData/config` 的 `runtimePreferences` 配置段：

- 当前格式是 `RuntimePreferences`。
- 旧 `userData/runtime-preferences.json` 只作为迁移输入；当 `userData/config.runtimePreferences` 已存在时，共享 config 配置段优先。
...
- `toolAvailability`、`command`、`compaction`、`skills`、`permissionRules`、`mcpServers` 和 `approvalExperience` 分别由 runtime 工具目录、命令工具、上下文压缩、skills 注入、权限决策、MCP host 与 renderer 展示消费。

Checkpoint 数据由 `CheckpointStore` 写入 Electron `userData/checkpoints/`：
...
- 每个 checkpoint 关联一个 `threadId`、`turnId`、workspace、prompt 摘要和文件快照摘要。
- 写入工具在 commit 边界记录 touched file 的 pre-edit 与 post-write 快照，支持 code rewind 与可选 session rewind。
- restore 必须重新执行 workspace 边界与符号链接路径校验；不得把 checkpoint 当成绕过 workspace policy 的后门。

MCP cache 由 `McpCacheStore` 写入 Electron `userData/mcp/cache.json`：

- cache 只保存公共 tool schema、prompt/resource descriptors、capability snapshot 与 startup stats。
- `RuntimePreferences.mcpServers` 是 MCP 配置权威来源；cache 只是加速和 lazy reconnect 输入，不能反向覆盖用户配置。
...
1. 在 `src/shared/agent-contracts.ts` 统一出口及对应 focused submodule 中定义 request / response 类型。
...
- `checkpoints`：list / rewind，支持 code rewind 和可选 session rewind。
- `workspace`：pickDirectory，使用 Electron dialog 选择目录。
- `write`：Markdown list / get / put / create / rename / delete / complete。
- `modelConfig`：get / update / profile list / create / update / delete / activate。
- `runtimePreferences`：get / update。
- `skills`：list，暴露只读 workspace skill catalog 诊断。
- `mcp`：server status/connect/disconnect、tools refresh/list、surface refresh、prompts get/list、resources read/list。
...
- 新事件必须同步 `RUNTIME_EVENT_KINDS`、`RuntimeEventKind`、`RuntimeEventBus.onThread()` 订阅列表、preload guard 和 renderer 消费逻辑。
- 现有事件包括 `turn_started`、`turn_completed`、`turn_failed`、`item_appended`、`item_updated`、`approval_requested`、`tool_progress`、`mcp_server_connection`、`mcp_tool_list_changed`、`mcp_surface_changed`、`tool_budget_reached`、`goal_updated`、`runtime_error`。
- `tool_progress` 是 live-only stdout/stderr 进度事件；最终可持久化结果仍以 `ToolItem.result` 为准。
- MCP runtime events 是 process-level events，不携带 `threadId`；SSE handler 会按订阅窗口转发，聊天时间线不得把它们当作 thread item。
...
- `npx vitest`：以 watch 模式运行 Vitest，适合本地迭代。
- `npm test -- <path>`：运行单个测试文件。
- `npm test -- -t "<name>"`：按测试名过滤。
...
Agent 开发维护入口位于 `docs/agent-development.md`，用于说明文档归属、开发流程和跨模块维护策略；它不再承载逐条变更记录。项目快速理解文档包括：
...
凡是修改以下能力，必须同步更新对应的权威文档：

- Agent 运行框架。
- LLM 接入。
- 工具机制。
- Skills / subagent playbook。
- MCP 外部工具 / prompts / resources。
- Checkpoint / rewind。
- IPC 契约。
- 持久化格式。
- 桌面 UI。
...
- 修改 `ThreadRecord`、`TurnRecord`、`Item`、`RuntimeEvent`、附件、模型配置、runtime preferences、checkpoint、MCP cache、JSONL 格式或迁移规则时，同步 `docs/data-model.md`。
- 修改设计 token、布局语法、主题、组件模式时同步 `docs/ui-design.md`；修改页面结构或组件布局时同步 `docs/ui-layout-reference.md`。
- 只有修改开发流程、文档归属或跨模块维护策略时，才同步 `docs/agent-development.md`。

若任务明确属于 OpenSpec change，且仓库中存在对应 `openspec/changes/<change-id>/`，优先检查其 `proposal.md`、`design.md`、`tasks.md` 和 `specs/`；实施前确认 `tasks.md` 未完成项，完成后同步勾选对应任务并保持规格文档与实现一致。当前仓库不存在该目录时，不要凭空创建或引用 OpenSpec 路径，除非用户明确要求启动 OpenSpec 工作流。
...
- 验证命令。
- 涉及 UI 时附截图。
- 涉及接口或协议时注明依据的源码、测试或 provider 文档。
- 涉及架构、IPC、数据模型、runtime、UI 或维护流程时说明已同步的权威文档；如无需更新，写明原因。
...
- 文件写入能力必须做路径边界检查；Write IPC 的 `resolveWritePathForAccess()` / `resolveWritePath()` 在 Markdown 扩展名约束外复用 `workspace-policy.ts` 的 realpath、path escape 和 skipped directory 防护。
...
- [ ] 如果涉及架构、IPC、数据模型、runtime、UI 或维护流程，是否已同步更新对应权威文档？
