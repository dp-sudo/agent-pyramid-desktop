...
- `openspec/`（当前仓库已存在时，作为变更提案、设计、任务和规格的工作流目录）
...
- `src/main/application/tools/`：工具注册与内置工具。`InMemoryToolRegistry` 是当前注册表；`createPlanTool`、`createWorkspaceTools()`、`createCodingTools()`、`createCommandTools()`、`createGoalTools()` 是当前 runtime 可用工具来源。
...
- 当前注册工具包括 `createPlanTool`、`createWorkspaceTools()` 返回的 `list_files` / `read_file` / `search_files`，`createCodingTools()` 返回的 `edit_file` / `write_file` / `apply_patch` / `rollback_file`，`createCommandTools()` 返回的 `run_command` / `diagnose_workspace` / `diagnose_file`，以及 `createGoalTools()` 返回的 `update_goal`。
- 只读 workspace 工具和 `diagnose_file` 跳过 approval；`create_plan` / `update_goal` 由模式门控并跳过 approval；coding 写入工具、`run_command` 和 `diagnose_workspace` 进入 approval gate。
...
涉及 OpenSpec change 且仓库中存在对应目录时，优先检查 `openspec/changes/` 下对应 change 的 `proposal.md`、`design.md`、`tasks.md` 和 `specs/`；实施前确认 `tasks.md` 未完成项，完成后同步勾选对应任务并保持规格文档与实现一致。
