...
- Write threads 默认隐藏并拒绝 Code-only coding/command 工具；tool access policy 可按 `code` / `write` mode allow 或 deny 指定工具名，但 approval 和 sandbox 策略仍在后续执行前生效。
- `RuntimePreferences.permissionRules` 可按 command / write / MCP 规则设置 `allow | ask | deny`，但 hard read-only sandbox 和 `approvalPolicy: "never"` 的拒绝优先。
...
- 修改工具策略时必须同步 `AgentRuntime.listToolDefinitionsForTurn()`、`AgentRuntime.resolveToolPolicy()`、`AgentRuntime.requiresApproval()`、UI 和持久化记录。
...
涉及 OpenSpec change 时，优先检查 `openspec/changes/` 下对应 change 的 `proposal.md`、`design.md`、`tasks.md` 和 `specs/`；实施前确认 `tasks.md` 未完成项，完成后同步勾选对应任务并保持规格文档与实现一致。
