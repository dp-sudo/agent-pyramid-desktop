...
| --- | --- | --- |
| Main composition | `src/main/index.ts` | Wires stores, event bus, worker pool, runtime, tool registry, MCP host, IPC handlers, BrowserWindow. |
| Runtime | `src/main/application/agent-runtime.ts` | Turn lifecycle, model profile resolution, prompt/context assembly, worker calls, tool loop, interrupt, events. |
...
| Built-in tools | `src/main/application/tools/` | Workspace, coding, command, skill, user-input, plan, and goal tools. |
...
| Provider gateway | `src/main/infrastructure/minimax/` | `MiniMaxGateway`, OpenAI-compatible adapter, Anthropic-compatible adapter, SSE parsing. |
| MCP | `src/main/infrastructure/mcp/` | MCP client, host, transports, cache, dynamic tool registration, prompts/resources. |
...
| Shared contracts | `src/shared/agent-contracts.ts`, `src/shared/agent-api.ts`, `src/shared/ipc.ts`, `src/shared/ipc-errors.ts` | Cross-process types, preload API shape, channels, error codes. |
...
| Renderer shell | `src/renderer/src/ui/AppShell.tsx`, `src/renderer/src/ui/Workbench.tsx`, `src/renderer/src/ui/SettingsView.tsx` | Route shell, workbench flow, settings flow. |
...
| Change LLM protocol | `src/main/infrastructure/minimax/minimax-gateway.ts` | adapters, worker protocol, gateway tests |
...
| Change runtime preferences | `src/shared/agent-contracts.ts` | `RuntimePreferencesStore`, runtime, settings runtime preferences UI |
...
| Runtime preferences | `src/shared/agent-contracts.ts` | `RuntimePreferencesStore` in shared `userData/config` |
...
| Runtime / tools / policy / commands | `tests/main/application/` |
| Worker / gateway / MCP infrastructure | `tests/main/infrastructure/` |
| IPC handlers | `tests/main/ipc/` |
| Persistence stores | `tests/main/persistence/` |
| Skills | `tests/main/skills/` |
| Preload bridge | `tests/preload/` |
| Renderer reducer/components/helpers | `tests/renderer/` |
...
- Adding IPC without `RENDERER_TO_MAIN_CHANNELS`.
