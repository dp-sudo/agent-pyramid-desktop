...
- Made `start_command_session` wait for the child process to spawn before returning a session id, with pid, close/error, and timeout guards so missing shell/executable failures surface as tool errors instead of successful failed-session snapshots; runtime errors after a successful spawn still preserve `failed` session status across later child `close` events.
...
- Hardened single-file coding tool atomicity so `edit_file`, `write_file`, and `rollback_file` restore the pre-write file content if post-write metadata collection fails before file history/read state are updated.
- Hardened `apply_patch` multi-file atomicity so a file is registered for rollback immediately after its write succeeds, before post-write metadata collection; if `stat` fails after bytes are written, already updated/created files are restored and no file history is recorded.
- Verification plan: command tool tests cover cross-thread session denial, session spawn failure rejection, latest-output session buffering, session UTF-8 tail output, Windows `taskkill` fallback, deleted-file Git pathspecs, hidden and magic pathspec rejection, git log ref validation, package script-name validation, npm frozen-lockfile install behavior, workspace NUL string rejection, optional path rejection and numeric limit rejection, symlink-path coding tool rejection, single-file and multi-file post-write metadata rollback, invalid diagnostic package manifests, workspace-bound diagnostic paths, plus the existing command/coding tool safety regressions; full `typecheck/test/build` verification is run before handoff.
