...
- Store and renderer composer share `MAX_ATTACHMENT_BYTES` from
  ...
  uploading images over the same 12 MB limit that the store enforces. Persisted
  metadata must also report a positive size; zero-byte decoded payloads are not
  valid attachments.
...
| Channel | Preload Method | Request | Success Value | Error Codes |
| --- | --- | --- | --- | --- |
| `checkpoint:list` | `checkpoints.list(request)` | `CheckpointListRequest` | `CheckpointListResponse` | `THREAD_NOT_FOUND`, `CHECKPOINT_LIST_FAILED` |
| `checkpoint:rewind` | `checkpoints.rewind(request)` | `CheckpointRewindRequest` | `CheckpointRewindResponse` | `THREAD_NOT_FOUND`, `CHECKPOINT_REWIND_BUSY`, `CHECKPOINT_REWIND_FAILED` |

Notes:

- `checkpoint:list` validates the thread exists, reads checkpoint metadata, and
  derives `canRewindSession` from the current transcript turn ids. A checkpoint
  whose turn boundary is no longer present can remain code-rewindable while
  disabling session rewind in the renderer.
- `checkpoint:rewind` rejects in-flight threads. When `rewindSession` is true,
  the handler verifies the selected turn still exists in `messages.jsonl` before
...
- `write.rename` rejects identical source/target paths, reads the source through
  ...
  target, and removes the source only after the target write succeeds. If source
  removal fails, the handler removes the newly created target before returning
  `WRITE_RENAME_FAILED`, leaving the source as the retry handle.
...
- `mcp:servers:disconnect` keeps close errors visible through
  `MCP_SERVER_DISCONNECT_FAILED`, but the host clears local client/tool state
  first and leaves the server status as `disconnected` with `lastError`.
