...
- MCP tools are registered dynamically by `McpHost` using the
  ...
  selected target MCP tool (`server/tool`), not the facade tool name. Remote
  `annotations.readOnlyHint` is informational only; only local
  `RuntimePreferences.mcpServers[].readOnlyTools` marks an MCP tool or
  read-only facade target as read-only for approval and sandbox decisions.
