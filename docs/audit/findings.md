...
| SKILLS-DEFECT-001 | shared/main | defect | major | `src/shared/skills/registry.ts:42` | Workspace skill loading now treats invalid UTF-8 skill files and invalid `promptPatterns` as validation errors, and turn resolution uses the loaded workspace skill snapshot so concurrent resolves cannot read a registry mutated by another workspace load. |
